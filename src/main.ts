...
program
  ...
  .action((options) => {
    ...
    if (description && amount) {
      ...
      console.error(red('Both description and amount must be provided.'))
    }
  })
  ...
  .action(async (options) => {
    ...
    if (id?.toString()) {
      try {
        ...
        console.error(red('Expense deleted successfully'))
      } catch (error) {
        console.error((error as Error).message)
      }
    } else {
      console.error(red('Please provide an expense ID to delete'))
    }
  })
